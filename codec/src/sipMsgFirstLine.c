...
{ 
	...
	if(pFL->isReqLine)
	{
		...
		{
			if(pSipMsg->buf[flEnd++] == '\r')
			{
				if(pSipMsg->buf[flEnd++] = '\n')
				{
					flEnd -= 2;
					break;
				}
		...
		if(flEnd == pSipMsg->pos)
		{
			logError("SIP message does not contain proper request line.");
            status = OS_ERROR_INVALID_VALUE;
            goto EXIT;
        }
        ...
        {
            logError("The SIP request does not have correct SIP version or ending, pos=%ld.", pSipMsg->pos);
            status = OS_ERROR_INVALID_VALUE;
            goto EXIT;
        }
