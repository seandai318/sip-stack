...
//		DEBUG_SIP_PRINT_TOKEN(sippInfo[i].token, sippInfo[i].tokenNum);
		...
		debug("start parsing for: sbnfNum=%d, idx=%d, paramName=%d, sipABNF[i].Token='%c'(0x%x). Previous parsing: isEOH=%d, tokenMatched='%c'(0x%x)", sbnfNum, i, sipABNF[i].paramName, sipABNF[i].extToken, sipABNF[i].extToken, isEOH, tokenMatched, tokenMatched);
...
//				DEBUG_SIP_PRINT_TOKEN(sippInfo[i].token, sippInfo[i].tokenNum);
				...
				if(SIP_IS_MATCH_TOKEN(tokenMatched, sippInfo[i].token, sippInfo[i].tokenNum-sippInfo[i].extTokenNum, sippInfo[i].tokenNum))
				{
					...
					debug("sean, pSipMsg->pos=%ld, char='%c'", pSipMsg->pos, pSipMsg->buf[pSipMsg->pos]); 
...
//					debug("add local token (%c) to the token list, sippParsingInfo idx=%d, tokenNum=%d, extTokenNum=%d", sippParsingInfo[j].token[sippParsingInfo[j].tokenNum-1], j, sippParsingInfo[j].tokenNum, sippParsingInfo[j].extTokenNum);
    ...
    {
//    	debug("add parent token to token list, sippParsingInfo idx=%d, tokenNum=%d, extTokenNum=%d, parent tokenNum=%d.", i, sippParsingInfo[i].tokenNum, sippParsingInfo[i].extTokenNum, pParentParsingInfo->tokenNum);
