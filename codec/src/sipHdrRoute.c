...
void sipHdrRoute_cleanup(void* data)
{
    ...
    sipHdrRoute_t* pRoute = data;
    osList_t* pRouteList = &pRoute->routeList;
    osListElement_t* pRouteLE = pRouteList->head;
    while(pRouteLE)
    {
        sipHdrGenericNameParam_cleanup(pRouteLE->data);

        pRouteLE = pRouteLE->next;
    }

    osList_delete(pRouteList);
}


void* sipHdrRoute_alloc()
{
    sipHdrRoute_t* pRoute = osMem_alloc(sizeof(sipHdrRoute_t), sipHdrRoute_cleanup);
    if(!pRoute)
    {
        return NULL;
    }

    osList_init(&pRoute->routeList);
