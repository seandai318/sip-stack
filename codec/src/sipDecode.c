...
sipMsgDecodedRawHdr_t* sipDecodeMsgRawHdr(osMBuf_t* pSipMsg, sipHdrName_e sipHdrArray[], int hdrArraySize)
{
    ...
    //the assumption is that pSipMsg->pos points to the beginning of sip hdr field
    if(pSipMsg->pos == 0)
    {
        logError("pSipMsg->pos = 0, this funtion shall only be called after the firstLine parse and the pSipMsg->pos is set to the beginning of the hdr fields.");
        status = OS_ERROR_NULL_POINTER;
        goto EXIT;
    }
...
//			osList_init(&pHdr->decodedHdrList);
		...
		if(pHdr->rawHdr.pRawHdr == NULL)
		{
			pHdr->rawHdr.pRawHdr == pSipHdr;
			pHdr->rawHdr.rawHdrNum = 1;
		}
		else
		{
			osList_append(&pHdr->rawHdr.rawHdrList, pSipHdr);
			++pHdr->rawHdr.rawHdrNum;
		}
...
static void sipMsgDecoded_delete(void *data)
{
    ...
    while (pLE)
    {
	    ...
	    osMem_deref(pHdrInfo->rawHdr.pRawHdr);
    	osList_delete(&pHdrInfo->rawHdr.rawHdrList);
...
static void sipHdrListMemDestroy(void* data)
{
	...
	osList_delete(&pSipHdrInfo->decodedHdrList);
    //to-do, double check if this line shall be called. decodeValue may refer field in sipBuf    osMem_deref(pHdrInfo->decodedValue);
} 	
