	...
	switch(pNMT->hdrCode)
	{
		...
		case SIP_HDR_VIA:
			...
			if(pNMT->modType == SIP_HDR_MODIFY_TYPE_ADD)
			{
                ...
                osList_append(&viaHdr.viaParamList, &branch);
			...
			else if (pNMT->modType == SIP_HDR_MODIFY_TYPE_REPLACE)
			{
				...
				sipHdrViaElement_t* pViaHdr= viaHdrDecoded.decodedValue;
				debug("to remove, pViaHdr=%p", pViaHdr);
				debug("to remove, via.host=%r, port=%r.", &pViaHdr->hostport.host, &pViaHdr->hostport.port);
				sipHdrParamNameValue_t param1={{"received", 8}, {"10.11.22.33", 11}};
				sipHdrParamNameValue_t param2={{"rport", 5}, {"5060", 4}};
                osList_append(&pViaHdr->viaParamList, &param1);
                osList_append(&pViaHdr->viaParamList, &param2);
